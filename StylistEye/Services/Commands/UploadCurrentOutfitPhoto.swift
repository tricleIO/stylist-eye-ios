 ...
 */
struct UploadCurrentOutfitPhotoCommand2: NetworkExecutable, UploadQueueItem {
  ...
  init(id: Int, image: UIImage, imageData: Foundation.Data) {
    urlManager = .uploadCurrentOutfitPhoto2(id: id, photo: imageData)
    self.image = image
    self.imageData = imageData
    self.id = id
  }
